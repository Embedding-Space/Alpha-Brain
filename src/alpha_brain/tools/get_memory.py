...
async def get_memory(memory_id: str) -> str:
    ...
    if memory.extra_data:
        ...
        summary = memory.extra_data.get("summary", "")
        if summary:
            result += f"Summary: {summary}\n"

        emotional_tone = memory.extra_data.get("emotional_tone", "")
        if emotional_tone:
            result += f"Emotional Tone: {emotional_tone}\n"

        importance = memory.extra_data.get("importance", None)
        if importance is not None:
            result += f"Importance: {importance}/5\n"

        people = memory.extra_data.get("people", [])
        if people:
            result += f"People: {', '.join(people)}\n"

        technologies = memory.extra_data.get("technologies", [])
        if technologies:
            result += f"Technologies: {', '.join(technologies)}\n"

        organizations = memory.extra_data.get("organizations", [])
        if organizations:
            result += f"Organizations: {', '.join(organizations)}\n"

        places = memory.extra_data.get("places", [])
        if places:
            result += f"Places: {', '.join(places)}\n"

        keywords = memory.extra_data.get("keywords", [])
        if keywords:
            result += f"Keywords: {', '.join(keywords)}\n"

        # Show analyzed_at if available
        analyzed_at = memory.extra_data.get("analyzed_at", "")
        if analyzed_at:
