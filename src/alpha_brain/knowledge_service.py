...
class KnowledgeService:
    ...
    async def create(self, knowledge_input: KnowledgeInput) -> KnowledgeOutput:
        ...
        if existing:
            raise ValueError(f"Knowledge document with slug '{knowledge_input.slug}' already exists")
    ...
    async def get_by_slug(self, slug: str) -> KnowledgeOutput | None:
        ...
        """
        result = await self.db.execute(
            select(Knowledge).where(Knowledge.slug == slug)
        )
        knowledge = result.scalar_one_or_none()
    ...
    async def update(self, slug: str, knowledge_input: KnowledgeInput) -> KnowledgeOutput | None:
    ...
    async def list_all(self, limit: int = 100, offset: int = 0) -> list[KnowledgeOutput]:
